...
def get_kk_img(sum_mv: tuple[int, int]) -> pg.Surface:
    ...
    kk_imgs = {
        ...
        (5, 0): pg.transform.rotozoom(pg.image.load("fig/3.png"), 0, 1.0),  # 右向き
        (-5, 0): pg.transform.rotozoom(pg.image.load("fig/3.png"), 180, 1.0),  # 左向き
        (0, 5): pg.transform.rotozoom(pg.image.load("fig/3.png"), 90, 1.0),  # 下向き
        (0, -5): pg.transform.rotozoom(pg.image.load("fig/3.png"), -90, 1.0),  # 上向き
        (5, 5): pg.transform.rotozoom(pg.image.load("fig/3.png"), 45, 1.0),  # 右下
        (5, -5): pg.transform.rotozoom(pg.image.load("fig/3.png"), -45, 1.0),  # 右上
        (-5, 5): pg.transform.rotozoom(pg.image.load("fig/3.png"), 135, 1.0),  # 左下
        (-5, -5): pg.transform.rotozoom(pg.image.load("fig/3.png"), -135, 1.0),  # 左上
    }
...
def main():
    ...
    bg_img = pg.transform.scale(bg_img, (WIDTH, HEIGHT))  # スクリーンサイズに合わせる

    # こうかとん画像
    kk_img = pg.transform.rotozoom(pg.image.load("fig/3.png"), 0, 0.9)
    kk_rct = kk_img.get_rect()
    kk_rct.topleft = (300, 200)  # 初期位置を設定
    ...
    bb_rct.topleft = (random.randint(0, WIDTH-20), random.randint(0, HEIGHT-20))  # ランダムな初期位置
